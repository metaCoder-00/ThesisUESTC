...
\title{数字相控阵单脉冲测角方法研究}{The Research on Monopulse Angle Measurement for 
Digital Phased Array}

\author{邓宇昊}{Yuhao Deng}
\advisor{谢菊兰\chinesespace 副教授}{Dr. Julan Xie}
\school{信息与通信工程学院}{School of Communication and Information Engineering}
...
最直接的方法是利用阵列的空间功率谱，查找整个空域中功率最大的方向，将其作为期望信号入射方向的估计值\citing{Capon}。
这种较为原始的方法实现起来简单，但容易受到各类因素的影响，且测角误差较大。第二类方法是以MUSIC方法为代表的空间谱方法\citing{Schmidt}，
这类方法利用信号子空间与噪声子空间的正交性，构建阵列的空间谱，搜索谱峰得到期望信号方向。
该方法从信号子空间和噪声子空间的角度入手，突破了阵列的瑞利极限，遂称之为超分辨率算法。除此以外，
空间谱方法的代表算法还有root-MUSIC和ESPRIT方法等\citing{Krim,Roy}。虽然空间谱方法具有较高的分辨率，但这类方法通常涉及特征分解，
计算量较大，并且大部分空间谱方法都必须预知入射信源的个数，因此必须与一些估计信源个数的方法相结合\citing{Wax}，才能进行后续的测向过程。
...
传统单脉冲测向方法主要有三种\citing{Sun}。第一种是半阵法\citing{Xun}，该方法只能用于均匀线阵和均匀面阵，
...
T. Taylor于1960年提出了针对圆形和线性天线孔径的低旁瓣和波束设计方法\citing{Taylor_55,Taylor}。
E. T. Bayliss于1968年提出了针对圆形和线性天线孔径的低旁瓣差波束设计方法\citing{Bayliss}。
接着P. Darwood将Bayliss的差波束设计方法从连续的天线孔径拓展到多天线相控阵上\citing{Darwood}。
类似的，Taylor的和波束设计方法也从连续孔径拓展到了数字相控上\citing{Folgueiras}。
...
而第三种传统单脉冲测向方法——比幅法则可以用于共形阵\citing{Mosca}。这类方法构造对每个角度通道构造两个波束，
...
用于设计低旁瓣的和差波束权。比如设计优化目标函数与约束条件的数学优化方法\citing{Bucci,Isernia,Rocca}，
以及利用遗传类算法进行和差波束设计\citing{Lopez,Johnson,Caorsi}。
其次是广义旁瓣对消器\citing{Buckley,Bitzer,Jablon,Zhou}，这类方法利用一个辅助阵列（一般情况下维度小于主阵列），
...
还有一些研究借助LCMV波束形成器的思想\citing{Hadad}，提出了一种线性约束方法\citing{Cheng,Zhu}，这类方法约束单脉冲比，
以最小均方误差为原则，结合MVDR波束形成器\citing{Wolfel}，利用拉格朗日乘子法解得自适应差波束权，
...
U. Nickel 在 1993 年将该方法应用到了单脉冲系统中\citing{Nickel_93}。
...
注意到，\eqref{JLC_opt_obj_a}和\eqref{JLC_opt_obj_e}的结构和LCMV波束形成器类似\citing{Hadad}。
...
使得单脉冲比$f$在每个点上都满足线性关系。但从LCMV波束形成器的原理中可知\citing{Hadad}，约束点数会受到阵列自由度的限制，
...
线性约束方法的测角误差如图\ref{jlc_rmse}所示，SVD-线性约束方法的测角误差如图\ref{svd_jlc_rmse}所示。
\begin{figure}[H]
    \subfloat[]{
        \label{ml_azm_rmse}
        \includegraphics[scale=0.288]{pic/ml_azm_rmse.eps}
    }
    \subfloat[]{
        \label{ml_elv_rmse}
        \includegraphics[scale=0.288]{pic/ml_elv_rmse.eps}
    }
...
\begin{figure}[H]
    \subfloat[]{
        \label{mvam_azm_rmse}
        \includegraphics[scale=0.288]{pic/mvam_azm_rmse.eps}
    }
    \subfloat[]{
        \label{mvam_elv_rmse}
        \includegraphics[scale=0.288]{pic/mvam_elv_rmse.eps}
    }
    \caption{存在主旁瓣干扰时MVAM方法方向估计的RMSE。(a)方位角RMSE；(b)俯仰角RMSE}
    \label{mvam_rmse}
\end{figure}
...
\begin{figure}[H]
    \subfloat[]{
        \label{jlc_azm_rmse}
        \includegraphics[scale=0.288]{pic/jlc_azm_rmse.eps}
    }
    \subfloat[]{
        \label{jlc_elv_rmse}
        \includegraphics[scale=0.288]{pic/jlc_elv_rmse.eps}
    }
...
\begin{figure}[H]
    \subfloat[]{
        \label{svd_jlc_azm_rmse}
        \includegraphics[scale=0.288]{pic/svd_jlc_azm_rmse.eps}
    }
    \subfloat[]{
        \label{svd_jlc_elv_rmse}
        \includegraphics[scale=0.288]{pic/svd_jlc_elv_rmse.eps}
    }
...
\begin{figure}[H]
    \subfloat[]{
        \label{jlc_azm_rmse_slj}
        \includegraphics[scale=0.288]{pic/jlc_azm_rmse_slj.eps}
    }
    \subfloat[]{
        \label{svd_jlc_azm_rmse_slj}
        \includegraphics[scale=0.288]{pic/svd_jlc_azm_rmse_slj.eps}
    }
...
\begin{figure}[H]
    \subfloat[]{
        \label{jlc_elv_rmse_slj}
        \includegraphics[scale=0.288]{pic/jlc_elv_rmse_slj.eps}
    }
    \subfloat[]{
        \label{svd_jlc_elv_rmse_slj}
        \includegraphics[scale=0.288]{pic/svd_jlc_elv_rmse_slj.eps}
    }
...
MVAM方法预设理想的单脉冲比，然后在波束指向处进行一阶泰勒展开，经过一系列化简后，
...
这样就可以利用最大似然方法得到两个通道信号复振幅的估计值\citing{Ma}。
...
\begin{equation}\label{polar_lf_opt}
    \begin{aligned}
        ...
        &\text{s.t.} ~ -\pi\leqslant\theta<\pi, 0\leqslant\phi\leqslant\pi
    \end{aligned}
\end{equation}
...
本方法将与最大似然方法\citing{Nickel_93}和比幅法\citing{Mosca}进行比较。
...
\begin{figure}[H]
    \subfloat[]{
        ...
        \includegraphics[scale=0.288]{pic/azimuth_RMSE_jammer_free.eps}
    }
    \subfloat[]{
        \label{Polar_RMSE_jammer_free_b}
        \includegraphics[scale=0.288]{pic/elevation_RMSE_jammer_free.eps}
    }
...
\begin{figure}[H]
    \subfloat[]{
        ...
        \includegraphics[scale=0.288]{pic/azimuth_RMSE_SLJ.eps}
    }
    \subfloat[]{
        \label{Polar_RMSE_SLJ_b}
        \includegraphics[scale=0.288]{pic/elevation_RMSE_SLJ.eps}
    }
...
\begin{figure}[H]
    \subfloat[]{
        ...
        \includegraphics[scale=0.288]{pic/gamma_hat.eps}
    }
    \subfloat[]{
        \label{Polar_args_RMSE_b}
        \includegraphics[scale=0.288]{pic/eta_hat.eps}
    }
...
\begin{figure}[H]
    \subfloat[]{
        \label{Polar_args_RMSE_a_SLJ}
        \includegraphics[scale=0.288]{pic/gamma_hat_SLJ.eps}
    }
    \subfloat[]{
        \label{Polar_args_RMSE_b_SLJ}
        \includegraphics[scale=0.288]{pic/eta_hat_SLJ.eps}
    }
